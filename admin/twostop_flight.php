...
<form action="twostop.php" method="post">
    <h2>Two Stop Details</h2>
    Flight Name:
    <select name="flight_id" id="flight_id" required>
        <option></option>
        <?php
        $flight_query = "SELECT * FROM flight where `stop`='two stop' and `status`= 0";
...
<script>
    function addStop() {
        ...
        if (selectedFlightId !== '') {
            window.location.href = 'twostop.php?flight_id=' + selectedFlightId;
        } else {
